...
class UltrafastLaneDetectorV2(LaneDetectBase):
	...
	def __prepare_input(self, image : cv2) -> np.ndarray :
			...
			# ==========================================
			# ADAS PERSPECTIVE CALIBRATION 
			# ==========================================
			# 70mai has a huge FOV. We crop the frame to simulate 
			# the narrower TuSimple camera perspective.
			self.crop_top = int(self.img_height * 0.25)    # Cut sky & raise the vanishing point
			self.crop_bottom = int(self.img_height * 0.10) # Cut the car hood
			self.crop_left = int(self.img_width * 0.15)    # Cut left fish-eye distortion
			self.crop_right = int(self.img_width * 0.15)   # Cut right fish-eye distortion

			# Extract the true "Road" Region of Interest
			img_roi = img[self.crop_top : self.img_height - self.crop_bottom, 
										self.crop_left : self.img_width - self.crop_right]
			
			self.roi_height, self.roi_width, _ = img_roi.shape

			# Resize the calibrated ROI, NOT the stretched wide-angle image
			new_size = ( self.input_width, int(self.input_height/self.cfg.crop_ratio))
			img_input = cv2.resize(img_roi, new_size).astype(np.float32)
			img_input = img_input[-self.input_height:, :, :]
	...
	# 	output = output[0] if isinstance(output, list) and len(output) == 1 else output
	...
	def __process_output(self, output, cfg : ModelConfig, local_width :int = 1) -> Tuple[np.ndarray, list]:
			# Calculate mapping based on the ROI, not the original 4K width
			roi_image_width = self.roi_width
			roi_image_height = self.roi_height
				...
				if valid_row[0,:,i].sum() > num_cls_row / 2:
					for k in range(valid_row.shape[1]):
						if valid_row[0,k,i]:
							...
							# Map math back to ROI, then ADD crop offsets to project onto Full Frame
							x_coord = int(out_tmp / (num_grid_row-1) * roi_image_width) + self.crop_left
							y_coord = int(cfg.row_anchor[k] * roi_image_height) + self.crop_top
				...
				if valid_col[0,:,i].sum() > num_cls_col / 4:
					for k in range(valid_col.shape[1]):
						if valid_col[0,k,i]:
							...
							# Map math back to ROI, then ADD crop offsets to project onto Full Frame
							y_coord = int(out_tmp / (num_grid_col-1) * roi_image_height) + self.crop_top
							x_coord = int(cfg.col_anchor[k] * roi_image_width) + self.crop_left
